...
func TestActivitiesRequestedGTAvailable(t *testing.T) {
	...
	a.NoError(err)
	ctx := context.Background()
	acts, err := client.Activity.Activities(ctx, common.Pagination{Total: 325})
	a.NoError(err)
	a.Equal(2, len(acts))
}
...
func TestActivitiesMany(t *testing.T) {
	...
	acts, err := client.Activity.Activities(ctx, common.Pagination{Total: 127, Start: 0, Count: 1})
	a.NoError(err)
	a.NotNil(acts)
	...
	acts, err = client.Activity.Activities(ctx, common.Pagination{Total: x, Start: 0})
	a.NoError(err)
	a.NotNil(acts)
	a.Equal(n, len(acts))
	...
	acts, err = client.Activity.Activities(ctx, common.Pagination{Total: 27, Start: 0})
	a.NoError(err)
	a.NotNil(acts)
	a.Equal(27, len(acts))
	...
	for _, x = range []int{27, 350, strava.PageSize} {
		acts, err = client.Activity.Activities(ctx, common.Pagination{Total: x, Start: 0, Count: count})
		a.NoError(err)
		a.NotNil(acts)
	...
	// negative test
	acts, err = client.Activity.Activities(ctx, common.Pagination{Total: -1})
	a.Error(err)
	a.Nil(acts)
}
