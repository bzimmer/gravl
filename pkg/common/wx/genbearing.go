...
func WindBearing(bearings []*Bearing) error {
	w := new(bytes.Buffer)
	fmt.Fprintln(w, "// Code generated by \"genbearing.go\"; DO NOT EDIT.")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "package wx")
	fmt.Fprintln(w, "import \"fmt\"")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "// WindBearing returns the compass direction for the wind bearing name")
	fmt.Fprintln(w, "func WindBearing(bearing string)(float64, error) { // nolint:gocyclo")
	fmt.Fprintln(w, "switch(bearing) {")
	for _, b := range bearings {
		fmt.Fprintf(w, "case \"%s\": // %s\n", b.Abbreviation, b.CompassPoint)
		fmt.Fprintf(w, "return %0.2f, nil\n", b.Bounds[1])
	}
	fmt.Fprintln(w, "default:")
	fmt.Fprintln(w, "return 0.0, fmt.Errorf(\"unknown bearing: %s\", bearing)")
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w, "}")
